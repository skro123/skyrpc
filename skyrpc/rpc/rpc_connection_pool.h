#ifndef SKYRPC_RPC_CONNECTION_POOL_H
#define SKYRPC_RPC_CONNECTION_POOL_H
#include "skyrpc/traits.h"
...
public:
    ...
    Result<R> call(const std::string& name, Params... ps) {
        ...
        // 从连接池里取出服务连接
        // std::cout << m_conns.size() << std::endl;
    ...
    void subscribe(const std::string& key, Func func) {
        {
            ...
            if (it != m_subHandle.end()) {
                SKYRPC_ASSERT2(false, "duplicated subscribe");
                return;
            }
...
#endif //SKYRPC_RPC_CONNECTION_POOL_H
