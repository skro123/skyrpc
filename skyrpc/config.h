#ifndef SKYRPC_CONFIG_H
#define SKYRPC_CONFIG_H
...
public:
    ...
    std::string toString() override {
        try {
            ...
            SKYRPC_LOG_ERROR(SKYRPC_LOG_ROOT())<<"ConfigVar::toString() exception "
    ...
    bool fromString(const std::string& str) override {
        try {
            ...
            SKYRPC_LOG_ERROR(SKYRPC_LOG_ROOT())<<"ConfigVar::fromString() exception "
...
public:
    ...
    static typename ConfigVar<T>::ptr Lookup(const std::string& name,const T& value,const std::string& description){
        ...
        if(it != GetDatas().end()){
            ...
            if(tmp){
                SKYRPC_LOG_INFO(SKYRPC_LOG_ROOT())<<"lookup name="<<name<<" already exist";
                return tmp;
            }else{
                SKYRPC_LOG_ERROR(SKYRPC_LOG_ROOT())<<"lookup name="<<name<<" already exist but type not "
        ...
        if(name.find_first_not_of("abcdefghijklmnopqrstuvwxyz0123456789._")
                != std::string::npos){
            SKYRPC_LOG_ERROR(SKYRPC_LOG_ROOT())<<"lookup invalid name "<<name;
