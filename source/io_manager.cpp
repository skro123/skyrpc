...
IOManager::FdContext::EventContext &IOManager::FdContext::getContext(IOManager::Event event) {
    ...
    switch(event) {
        ...
        case IOManager::WRITE:
            return write;
        default:
            SKYRPC_ASSERT2(false, "getContext");
    }
...
void IOManager::FdContext::triggerEvent(IOManager::Event event) {
    //SKYRPC_ASSERT2(events & event,std::to_string(event)+" & "+std::to_string(events)+" = "+std::to_string(events & event));
    if(!(events & event)){
        SKYRPC_LOG_ERROR(SKYRPC_LOG_ROOT()) << "ASSERTION: " << (events & event)
...
bool IOManager::addEvent(int fd, IOManager::Event event, std::function<void()> cb) {
    //SKYRPC_LOG_DEBUG(g_logger) << "addEvent() : fd=" << fd << " event=" << (event==1? "read" : "write");
    ...
    if(fdContext->events & event){
        SKYRPC_LOG_ERROR(g_logger) << "fd=" << fd << " addEvent fail, event already register. "
                << "event=" << event << " FdContext->event=" << fdContext->events;
        SKYRPC_ASSERT(!(fdContext->events & event));
    }
    ...
    if(rt){
        SKYRPC_LOG_ERROR(g_logger) << "epoll_ctl(" << m_epfd << ", " << op << ", " << fd << ", "
...
bool IOManager::cancelAllEvent(int fd) {
    ...
    SKYRPC_ASSERT(fdContext->events == 0);
    return true;
}
...
void IOManager::wait() {
    //SKYRPC_LOG_DEBUG(g_logger) << "wait for event";
    ...
    while (true){
        ...
        if(stopping(next_timeout)){
            SKYRPC_LOG_INFO(g_logger) << "name=" << getName()
                                     << " idle stopping exit";
            return;
        }
        int rt = 0;
        ...
        for (int i = 0; i < rt; ++i) {
            ...
            if(res){
                SKYRPC_LOG_ERROR(g_logger) << "epoll_ctl(" << m_epfd << ", " << op << ", " << fdContext->fd << ", "
