...
#include "skyrpc/raft/unstable.h"
...
void Unstable::truncateAndAppend(const std::vector<Entry>& ents) {
    ...
    if (after == offset + (int64_t)entries.size()) {
        ...
        SKYRPC_LOG_INFO(g_logger) << "replace the unstable entries from index " << after;
        // 这种情况存储可靠存储的日志还没有被提交，此时新的leader不在认可这些日志，所以替换追加
        offset = after;
        entries = ents;
    } else {
        SKYRPC_LOG_INFO(g_logger) << "truncate the unstable entries before index " << after;
...
void Unstable::mustCheckOutOfBounds(int64_t low, int64_t high) const {
    if (low < high) {
        SKYRPC_LOG_FMT_FATAL(g_logger, "invalid unstable.slice %ld > %ld", low, high);
    }
    int64_t upper = offset + entries.size();
    if (low < offset || high > upper) {
        SKYRPC_LOG_FMT_FATAL(g_logger, "unstable.slice[%ld,%ld) out of bound [%ld,%ld]", low, high, offset, upper);
    }
}
}