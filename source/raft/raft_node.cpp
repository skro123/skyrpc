...
#include "skyrpc/raft/raft_node.h"
#include "skyrpc/config.h"

namespace skyrpc::raft {
static skyrpc::Logger::ptr g_logger = SKYRPC_LOG_NAME("raft");
...
struct _RaftNodeIniter{
    _RaftNodeIniter(){
        ...
        g_timer_election_base->addListener([](const uint64_t& old_val, const uint64_t& new_val){
            SKYRPC_LOG_INFO(g_logger) << "raft election timeout base changed from "
                                    << old_val << " to " << new_val;
        ...
        g_timer_election_top->addListener([](const uint64_t& old_val, const uint64_t& new_val){
            SKYRPC_LOG_INFO(g_logger) << "raft election timeout top changed from "
                                    << old_val << " to " << new_val;
        ...
        g_timer_heartbeat->addListener([](const uint64_t& old_val, const uint64_t& new_val){
            SKYRPC_LOG_INFO(g_logger) << "raft heartbeat timeout changed from "
                                    << old_val << " to " << new_val;
...
void RaftNode::replicateOneRound(int64_t peer) {
    ...
    if (term < 0 || err) {
        ...
        if (!snapshot || snapshot->empty()) {
            SKYRPC_LOG_FATAL(g_logger) << "need non-empty snapshot";
        }
...
uint64_t RaftNode::GetRandomizedElectionTimeout() {
    static std::default_random_engine engine(skyrpc::GetCurrentMS());
