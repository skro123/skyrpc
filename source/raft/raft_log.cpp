#include "skyrpc/raft/raft_log.h"

namespace skyrpc::raft {
static Logger::ptr g_logger = SKYRPC_LOG_NAME("raft");

RaftLog::RaftLog(Storage::ptr storage, int64_t maxNextEntsSize /* = NO_LIMIT*/ ) {
    if (!storage) {
        SKYRPC_LOG_FATAL(g_logger) << "storage must not be nil";
    }
    m_storage = storage;
    ...
    if (first_index < 0 || last_index < 0) {
        SKYRPC_LOG_FATAL(g_logger) << "get index error";
    }
...
int64_t RaftLog::append(const std::vector<Entry>& entries) {
    ...
    if (after < m_committed) {
        SKYRPC_LOG_FMT_FATAL(g_logger, "after(%d) is out of range [committed(%d)]", after, m_committed);
    }
...
std::vector<Entry> RaftLog::nextEntries() {
    ...
    if (m_committed + 1 > off) {
        ...
        if (err) {
            SKYRPC_LOG_FATAL(g_logger) << "unexpected error when getting unapplied entries";
        }
        return ents;
    }
    return {};
...
int64_t RaftLog::firstIndex() {
    ...
    if (index < 0) {
        SKYRPC_LOG_FATAL(g_logger) << "unexpected error when getting first index";
    }
    return index;
}
...
int64_t RaftLog::lastIndex() {
    ...
    if (index < 0) {
        SKYRPC_LOG_FATAL(g_logger) << "unexpected error when getting last index";
    }
    return index;
}
...
int64_t RaftLog::lastTerm() {
    ...
    if (t < 0) {
        SKYRPC_LOG_FATAL(g_logger) << "unexpected error when getting the last term";
    }
    return t;
}
...
int64_t RaftLog::term(int64_t index) {
    ...
    SKYRPC_LOG_FATAL(g_logger) << "unexpected error when getting term";
    return 0;
}
...
std::vector<Entry> RaftLog::allEntries() {
    ...
    SKYRPC_LOG_FATAL(g_logger) << "unexpected error when getting allEntries";
    return {};
}
...
std::pair<std::vector<Entry>, StorageError> RaftLog::slice(int64_t low, int64_t high, int64_t maxSize) {
    ...
    if (low < m_unstable.offset) {
        ...
        if (err == StorageError::Compacted) {
            ...
            SKYRPC_LOG_FATAL(g_logger) << "entries[" << low << ":"
                << std::min(high, m_unstable.offset) << ") is unavailable from storage";
        } else if (err) {
            SKYRPC_LOG_FATAL(g_logger) << "unexpected error when slice";
        }
...
StorageError RaftLog::mustCheckOutOfBounds(int64_t low, int64_t high) {
    if (low > high) {
        SKYRPC_LOG_FATAL(g_logger) << "invalid slice " << low << " > " << high;
    }
...
int64_t RaftLog::zeroTermOnErrCompacted(int64_t term) {
    ...
    SKYRPC_LOG_FATAL(g_logger) << "unexpected error " << term;
    return 0;
}
