...
std::pair<std::vector<Entry>, StorageError> MemoryStorage::entries(int64_t low, int64_t high, int64_t maxSize) {
    ...
    if (high > LastIndex() + 1) {
        SKYRPC_LOG_FMT_FATAL(g_logger, "entries high(%ld) is out of bound last index(%ld)", high, LastIndex());
    }
    // 只有dummy日志
    if (m_entries.size() == 1) {
...
Snapshot::ptr MemoryStorage::CreateSnapshot(int64_t index, const std::string& data) {
    ...
    int64_t offset = m_entries.front().index;
    if (index > LastIndex()) {
        SKYRPC_LOG_FMT_FATAL(g_logger, "snapshot %ld is out of bound last index(%ld)", index, LastIndex());
    }
...
StorageError MemoryStorage::compact(int64_t compactIndex) {
    ...
    if (compactIndex > LastIndex()) {
        SKYRPC_LOG_FMT_FATAL(g_logger, "snapshot %ld is out of bound last index(%ld)", compactIndex, LastIndex());
    }
