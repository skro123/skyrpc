#include <fcntl.h>
#include <unistd.h>
#include "skyrpc/raft/snapshot.h"
...
Snapshotter::Snapshotter(const std::filesystem::path& dir,
    ...
    if (dir.empty()) {
        SKYRPC_LOG_WARN(g_logger) << "snapshot path is empty";
    }
    if (!std::filesystem::exists(m_dir)) {
        SKYRPC_LOG_WARN(g_logger) << "snapshot path: " << m_dir << " is not exists";
    }
    if (!std::filesystem::is_directory(m_dir)) {
        SKYRPC_LOG_WARN(g_logger) << "snapshot path: " << m_dir << " is not a directory";
    }
}
...
std::vector<std::string> Snapshotter::checkSffix(const std::vector<std::string>& names) {
    ...
    for (auto &name: names) {
        ...
        if (name.compare(name.size() - m_snap_suffix.size(), m_snap_suffix.size(), m_snap_suffix) == 0) {
            ...
            SKYRPC_LOG_WARN(g_logger) << "skipped unexpected non snapshot file " << name;
        }
    }
    return snaps;
}
