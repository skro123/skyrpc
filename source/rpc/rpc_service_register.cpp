...
struct _RpcRegistryIniter{
    _RpcRegistryIniter(){
        ...
        g_heartbeat_timeout->addListener([](const uint64_t& old_val, const uint64_t& new_val){
            SKYRPC_LOG_INFO(g_logger) << "rpc registry heartbeat timeout changed from "
                                    << old_val << " to " << new_val;
...
void RpcServiceRegistry::handleClient(Socket::ptr client) {
    ...
    while (true) {
        ...
        if (!request) {
            if (providerAddr) {
                SKYRPC_LOG_WARN(g_logger) << client->toString() << " was closed; unregister " << providerAddr->toString();
        ...
        switch (type) {
            ...
            case Protocol::MsgType::RPC_PROVIDER:
                SKYRPC_LOG_DEBUG(g_logger) << client->toString();
            ...
            case Protocol::MsgType::RPC_PUBLISH_RESPONSE:
                continue;
            default:
                SKYRPC_LOG_WARN(g_logger) << "protocol:" << request->toString();
                continue;
        }
