...
struct _RpcConnectionPoolIniter{
    _RpcConnectionPoolIniter(){
        ...
        g_channel_capacity->addListener([](const size_t& old_val, const size_t& new_val){
            SKYRPC_LOG_INFO(g_logger) << "rpc connection pool channel capacity changed from "
                                    << old_val << " to " << new_val;
...
void RpcConnectionPool::close() {
    SKYRPC_LOG_DEBUG(g_logger) << "close";
...
bool RpcConnectionPool::connect(Address::ptr address){
    ...
    if (!sock->connect(address, m_timeout)) {
        SKYRPC_LOG_ERROR(g_logger) << "connect to register fail";
        m_registry = nullptr;
        return false;
    }
    m_registry = std::make_shared<RpcSession>(sock);

    SKYRPC_LOG_DEBUG(g_logger) << "connect to registry: " << m_registry->getSocket()->toString();
    ...
    // 服务中心心跳定时器 30s
    m_heartTimer = skyrpc::IOManager::GetThis()->addTimer(30'000, [this]{
        SKYRPC_LOG_DEBUG(g_logger) << "heart beat";
        if (m_isHeartClose) {
            SKYRPC_LOG_DEBUG(g_logger) << "registry closed";
            //放弃服务中心
            m_heartTimer->cancel();
            m_heartTimer = nullptr;
        }
...
void RpcConnectionPool::handleSend() {
    ...
    while (m_chan >> request) {
        if (!request) {
            SKYRPC_LOG_WARN(g_logger) << "RpcConnectionPool::handleSend() fail";
            continue;
        }
        // 发送请求
...
void RpcConnectionPool::handleRecv() {
    ...
    while (true) {
        ...
        if (!response) {
            SKYRPC_LOG_WARN(g_logger) << "RpcConnectionPool::handleRecv() fail";
            close();
            break;
        }
        ...
        switch (type) {
            ...
            case Protocol::MsgType::RPC_SUBSCRIBE_RESPONSE:
                break;
            default:
                SKYRPC_LOG_DEBUG(g_logger) << "protocol:" << response->toString();
                break;
        }
    }
}
