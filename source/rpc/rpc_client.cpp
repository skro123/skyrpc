...
struct _RpcClientIniter{
    _RpcClientIniter(){
        ...
        g_channel_capacity->addListener([](const size_t& old_val, const size_t& new_val){
            SKYRPC_LOG_INFO(g_logger) << "rpc client channel capacity changed from "
                                    << old_val << " to " << new_val;
...
bool RpcClient::connect(Address::ptr address){
    ...
    if (m_auto_heartbeat) {
        m_heartTimer = IOManager::GetThis()->addTimer(30'000, [this]{
            ...
            if (m_isHeartClose) {
                SKYRPC_LOG_DEBUG(g_logger) << "Server closed";
                close();
            }
            // 创建心跳包
...
void RpcClient::handleSend() {
    ...
    while (m_chan >> request) {
        if (!request) {
            SKYRPC_LOG_WARN(g_logger) << "RpcClient::handleSend() fail";
            continue;
        }
        // 发送请求
...
void RpcClient::handleRecv() {
    ...
    while (true) {
        ...
        if (!response) {
            SKYRPC_LOG_WARN(g_logger) << "RpcClient::handleRecv() fail";
            close();
            break;
        }
        m_isHeartClose = false;
