...
struct _RpcServerIniter{
    _RpcServerIniter(){
        ...
        g_heartbeat_timeout->addListener([](const uint64_t& old_val, const uint64_t& new_val){
            SKYRPC_LOG_INFO(g_logger) << "rpc server heartbeat timeout changed from "
                                    << old_val << " to " << new_val;
...
bool RpcServer::bindRegistry(Address::ptr address) {
    ...
    if (!sock->connect(address)) {
        SKYRPC_LOG_WARN(g_logger) << "can't connect to registry";
        m_registry = nullptr;
        return false;
    }
...
bool RpcServer::start() {
    if (m_registry) {
        ...
        m_heartTimer = m_worker->addTimer(30'000, [server]{
            ...
            if (!response) {
                SKYRPC_LOG_WARN(g_logger) << "Registry close";
                //server->stop();
                //放弃服务中心，独自提供服务
        ...
        go [request, session, self, this]() mutable {
            ...
            switch (type) {
                ...
                case Protocol::MsgType::RPC_PUBLISH_RESPONSE:
                    return;
                default:
                    SKYRPC_LOG_DEBUG(g_logger) << "protocol:" << request->toString();
                    break;
            }
...
void RpcServer::update(Timer::ptr& heartTimer, Socket::ptr client) {
    // SKYRPC_LOG_DEBUG(g_logger) << "update heart";
    if (!heartTimer) {
        heartTimer = m_worker->addTimer(m_AliveTime, [client]{
            SKYRPC_LOG_DEBUG(g_logger) << "client:" << client->toString() << " closed";
            client->close();
        });
        return;
    }
...
void RpcServer::registerService(const std::string &name) {
    ...
    if (!rp) {
        SKYRPC_LOG_WARN(g_logger) << "registerService:" << name << " fail, registry socket:" << m_registry->getSocket()->toString();
        return;
    }
    ...
    if (result.getCode() != RPC_SUCCESS) {
        SKYRPC_LOG_WARN(g_logger) << result.toString();
    }
